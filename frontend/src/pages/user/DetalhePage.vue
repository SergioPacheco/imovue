<template>
  ...
  <div v-else>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Imagem -->
          <PropertyImage :tipo="imovel.tipoImovel" :numero="imovel.numeroImovel" :alt="`${imovel.tipoImovel || 'Imóvel'} da CAIXA em ${imovel.cidade}/${imovel.uf}`" size="lg">
        ...
        <div class="lg:col-span-1">
          <div class="sticky top-24 space-y-4">
            ...
            <div class="flex gap-2">
              ...
              <div class="relative">
                <button @click="showShare = !showShare" aria-label="Compartilhar imóvel" title="Compartilhar imóvel" class="btn-secondary px-3 py-2.5 hover:bg-brand-50 hover:border-brand-300 hover:text-brand-600 transition-all">
                ...
                <transition enter-active-class="transition ease-out duration-150" enter-from-class="opacity-0 scale-95 translate-y-1" enter-to-class="opacity-100 scale-100 translate-y-0" leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100 scale-100 translate-y-0" leave-to-class="opacity-0 scale-95 translate-y-1">
                  <div v-show="showShare" class="absolute right-0 mt-2 w-56 bg-white rounded-xl border border-gray-200 shadow-xl z-50 p-2 ring-1 ring-black/5 max-lg:fixed max-lg:right-3 max-lg:bottom-20 max-lg:mt-0">
                    <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider px-3 pt-1 pb-2">Compartilhar</div>
                    <button v-if="canNativeShare" @click="shareNative" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700 rounded-lg transition-colors w-full text-left">
                      <span class="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center text-brand-600">↗</span>
                      Compartilhar…
                    </button>
                    <a :href="shareWhatsApp" target="_blank" rel="noopener noreferrer" @click="trackShare('whatsapp')" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded-lg transition-colors">
                    ...
                    <a :href="shareTelegram" target="_blank" rel="noopener noreferrer" @click="trackShare('telegram')" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition-colors">
                    ...
                    <a :href="shareFacebook" target="_blank" rel="noopener noreferrer" @click="trackShare('facebook')" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 rounded-lg transition-colors">
                    ...
                    <a :href="shareLinkedIn" target="_blank" rel="noopener noreferrer" @click="trackShare('linkedin')" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition-colors">
                      <span class="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-sm">in</span>
                      LinkedIn
                    </a>
                    <a :href="shareX" target="_blank" rel="noopener noreferrer" @click="trackShare('x')" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 rounded-lg transition-colors">
    ...
    <div class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 flex gap-2 z-40 shadow-lg">
      ...
      <button @click="showShare = !showShare" aria-label="Compartilhar imóvel" title="Compartilhar imóvel" class="px-4 py-2.5 rounded-lg border border-gray-300 text-gray-600 text-sm">↗</button>
...
import { useSeoHead, getImovelSeo, getNotFoundSeo, SITE_URL } from '@/composables/useSeoHead'
...
function shareDisplayName(value: string | null | undefined) {
...
function shareCurrency(value: number | null | undefined) {
  if (value === null || value === undefined || Number.isNaN(Number(value))) return ''
  return Number(value).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })
}

const shareTitle = computed(() => {
  if (!imovel.value) return 'Imóvel da CAIXA com desconto'
  const im = imovel.value
  return `${shareDisplayName(im.tipoImovel || 'Imóvel')} da CAIXA em ${shareDisplayName(im.cidade || 'Brasil')}/${im.uf}`
})
const shareText = computed(() => {
  if (!imovel.value) return ''
  const im = imovel.value
  const details = [
    im.precoVenda ? `Preço: ${shareCurrency(im.precoVenda)}` : '',
    im.percentualDesconto ? `${Math.round(im.percentualDesconto)}% de desconto` : '',
    im.modalidadeVenda || '',
  ].filter(Boolean)
  return `${shareTitle.value}.${details.length ? ` ${details.join(' · ')}.` : ''} Imóvel nº ${im.numeroImovel}. Veja detalhes e condições no Imovue.`
})
const shareUrl = computed(() => `${SITE_URL}${window.location.pathname}`)
...
const shareFacebook = computed(() => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl.value)}&quote=${encodeURIComponent(shareText.value)}`)
const shareLinkedIn = computed(() => `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl.value)}`)
...
const trackShare = (network: string) => trackEvent('imovue_share', { network, property_id: imovel.value?.numeroImovel })
const shareNative = async () => {
  if (!imovel.value || !navigator.share) return
  try {
    await navigator.share({ title: shareTitle.value, text: shareText.value, url: shareUrl.value })
    trackShare('native')
    showShare.value = false
  } catch (error) {
    if ((error as DOMException).name !== 'AbortError') console.warn('Não foi possível compartilhar o imóvel.', error)
  }
}
